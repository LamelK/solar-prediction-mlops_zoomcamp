...
# Prometheus metrics
data_drift_gauge = Gauge('data_drift_share', 'Share of features with detected drift (Evidently)')
distance_drift_gauge = Gauge('distance_drift_share', 'Share of features exceeding distance threshold (Custom)')
statistical_drift_gauge = Gauge('statistical_drift_share', 'Share of features with statistically significant drift (p<0.05)')

# Enhanced drift metrics
enhanced_drift_gauge = Gauge('enhanced_drift_share', 'Share of features with enhanced drift detection (CRITICAL + WARNING)')
critical_drift_count = Gauge('critical_drift_count', 'Number of features with CRITICAL drift status')
warning_drift_count = Gauge('warning_drift_count', 'Number of features with WARNING drift status')
ok_drift_count = Gauge('ok_drift_count', 'Number of features with OK drift status')

# Level-specific drift metrics
distribution_drift_share = Gauge('distribution_drift_share', 'Share of features with distribution family issues')
parameter_drift_share = Gauge('parameter_drift_share', 'Share of features with parameter drift')
statistical_significance_share = Gauge('statistical_significance_share', 'Share of features with statistically significant drift')

# Individual feature drift status (0=OK, 1=WARNING, 2=CRITICAL, 3=ERROR)
feature_drift_status = Gauge('feature_drift_status', 'Drift status for individual features', ['feature_name'])

# Feature-specific detailed metrics
feature_mean_change = Gauge('feature_mean_change', 'Mean change percentage for features', ['feature_name'])
feature_std_change = Gauge('feature_std_change', 'Standard deviation change percentage for features', ['feature_name'])
feature_p_value = Gauge('feature_p_value', 'P-value from statistical test for features', ['feature_name'])
...
print(f"Monitoring interval: {MONITORING_INTERVAL} seconds ({MONITORING_INTERVAL/3600:.1f} hours)")
print(f"Statistical confidence level: {CONFIDENCE_LEVEL} (p < {CONFIDENCE_LEVEL} = significant)")
...
def check_statistical_significance(baseline_series, recent_series, feature_name):
    ...
    try:
        ...
        results['ks_test'] = {
            ...
            'interpretation': f"p={ks_p_value:.4f} {'< 0.05 (SIGNIFICANT)' if ks_p_value < CONFIDENCE_LEVEL else '> 0.05 (NOT SIGNIFICANT)'}"
        }
    ...
    try:
        if len(baseline_clean) > 20 and len(recent_clean) > 20:
            ad_stat, ad_critical, ad_significance = anderson_ksamp([baseline_clean, recent_clean])
            results['anderson_test'] = {
                'statistic': ad_stat,
                'significance': ad_significance,
                'significant': ad_significance < CONFIDENCE_LEVEL,
                'interpretation': f"p={ad_significance:.4f} {'< 0.05 (SIGNIFICANT)' if ad_significance < CONFIDENCE_LEVEL else '> 0.05 (NOT SIGNIFICANT)'}"
            }
        else:
            results['anderson_test'] = {
                'significant': False,
...
def enhanced_drift_analysis(baseline_series, recent_series, feature_name):
    ...
    results["distribution_family"] = {
        "status": "OK", 
        "message": f"Distribution check removed - focusing on parameters and significance"
    }
    ...
    try:
        ...
        baseline_scaled = scaler.fit_transform(baseline_clean.values.reshape(-1, 1)).flatten()
        recent_scaled = scaler.transform(recent_clean.values.reshape(-1, 1)).flatten()
        
        # Calculate statistics on scaled data
        baseline_mean_scaled, baseline_std_scaled = np.mean(baseline_scaled), np.std(baseline_scaled)
        recent_mean_scaled, recent_std_scaled = np.mean(recent_scaled), np.std(recent_scaled)
        
        # Calculate absolute changes on scaled data (fair comparison across features)
        # After scaling, baseline should have mean≈0 and std≈1, so we use absolute differences
        ...
        baseline_mean_orig, baseline_std_orig = np.mean(baseline_clean), np.std(baseline_clean)
        ...
        if combined_distance > SCALED_DISTANCE_THRESHOLD:
            ...
            param_message = f"Scaled distance: {combined_distance:.2f} > {SCALED_DISTANCE_THRESHOLD} (mean: {mean_change:.2f}, std: {std_change:.2f})"
        else:
            param_status = "OK"
            param_message = f"Scaled distance: {combined_distance:.2f} <= {SCALED_DISTANCE_THRESHOLD} (mean: {mean_change:.2f}, std: {std_change:.2f})"
    ...
    try:
        ...
        if p_value < 0.0001:
            ...
            p_display = f"{p_value:.6f}".rstrip('0').rstrip('.')  # Show exact value without trailing zeros
        
        results["statistical_test"] = {
            "status": "WARNING" if significant else "OK",
            "message": f"p={p_display} {'< 0.05 (SIGNIFICANT)' if significant else '> 0.05 (NOT SIGNIFICANT)'}",
            "p_value": p_value,
    ...
    # Only mark as drift if BOTH statistical significance AND scaled distance threshold are met
    ...
    results["combined_logic"] = {
        ...
        "both_conditions_met": p_value < CONFIDENCE_LEVEL and combined_distance > SCALED_DISTANCE_THRESHOLD
    }
...
def update_metrics():
    ...
    common_cols = [col for col in baseline.columns if col in recent.columns and col not in exclude_cols]
    ...
    for col in numeric_cols:
        ...
        print(f"  📈 Parameters: {enhanced_result['parameter_drift']['status']} - {enhanced_result['parameter_drift']['message']}")
        print(f"  🔬 Statistical: {enhanced_result['statistical_test']['status']} - {enhanced_result['statistical_test']['message']}")
        print(f"  🎯 Overall: {enhanced_result['overall_status']}")

    # Calculate enhanced drift metrics
    total_features = len(enhanced_results)
    enhanced_share = (critical_count + warning_count) / total_features if total_features > 0 else 0.0
    
    # Calculate level-specific drift shares
    distribution_issues = sum(1 for r in enhanced_results if r['distribution_family']['status'] in ['CRITICAL', 'WARNING'])
    parameter_issues = sum(1 for r in enhanced_results if r['parameter_drift']['status'] in ['CRITICAL', 'WARNING'])
    statistical_issues = sum(1 for r in enhanced_results if r['statistical_test']['status'] in ['CRITICAL', 'WARNING'])
    
    distribution_share = distribution_issues / total_features if total_features > 0 else 0.0
    parameter_share = parameter_issues / total_features if total_features > 0 else 0.0
    statistical_share = statistical_issues / total_features if total_features > 0 else 0.0
    ...
    for i, result in enumerate(enhanced_results):
        ...
        if 'mean_change' in result['parameter_drift']:
            feature_mean_change.labels(feature_name=feature_name).set(
                result['parameter_drift']['mean_change'] * 100  # Convert to percentage
            )
            feature_std_change.labels(feature_name=feature_name).set(
                result['parameter_drift']['std_change'] * 100  # Convert to percentage
            )
    ...
    # Determine overall drift status (simplified - no CRITICAL from distribution)
...
def compute_rmse_with_ground_truth(recent):
    ...
    merged = pd.merge(recent, ground_truth_df[['UNIXTime', 'Radiation']], on='UNIXTime', how='inner')
    if merged.empty:
    ...
    radiation_range = ground_truth_df['Radiation'].max() - ground_truth_df['Radiation'].min()
    ...
    print(f"  Note: Consider tracking RMSE trends over time for better context")
...
if __name__ == "__main__":
    ...
    print(f"Starting hourly drift monitoring...")
    while True:
