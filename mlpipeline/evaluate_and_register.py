...
def register_best_model(best_run, registry_model_name="MyTopModel"):
    ...
    logger = get_run_logger()
    logger.info(
        f"Registering best model: Run ID {best_run['run_id']} as {registry_model_name}"
    )
    ...
    for mv in client.search_model_versions(f"name='{registry_model_name}'"):
        if mv.version != version and mv.tags.get("status") == "production":
            client.set_model_version_tag(
                registry_model_name, mv.version, "status", "archived"
            )
...
def evaluate_and_register(logged_runs, X_test, y_test):
    ...
    for run in top_3_runs:
        ...
        test_results.append(
            {"run_id": run["run_id"], "model_name": run["model_name"], **test_metrics}
        )
    ...
    logger.info(
        f"Best model: {best_run['model_name']} | Run ID: {best_run['run_id']} "
        f"| Version: {version}"
    )
