...
def test_predict_json(mock_log_supabase, mock_preprocess, mock_model, sample_json_input):
...
@patch("api.serve_model.load_and_prepare_data")
@patch("api.serve_model.log_to_supabase")
def test_predict_csv(mock_log_supabase, mock_preprocess, mock_model, tmp_path, sample_json_input):
    ...
    with open(csv_path, "rb") as f:
        response = client.post("/predict_csv", files={"file": ("test.csv", f, "text/csv")})
